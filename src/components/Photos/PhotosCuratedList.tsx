...
const PhotosCuratedList = () => {
  ...
  const { data, loading, error, reFetch, clearData } = useFetch<CuratedPhotosData>(url, options, true, true);

  const [photosData, setPhotosData] = useState<Photo[] | undefined>(data?.photos);
